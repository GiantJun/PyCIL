...
class BiC(BaseLearner):
    ...
    def incremental_train(self, data_manager):
        ...
        if self._cur_task >= 1:
            ...
            self.val_loader = DataLoader(val_dset, batch_size=self._batch_size, shuffle=True, num_workers=self._num_workers)
    ...
    def _run(self, train_loader, test_loader, optimizer, scheduler, stage):
        for epoch in range(1, self._epochs+1):
            ...
            info = '{} => Task {}, Epoch {}/{} => Loss {:.3f}, Train_accy {:.3f}, Test_accy {:.3f}'.format(
                stage, self._cur_task, epoch, self._epochs, losses/len(train_loader), train_acc, test_acc)
            logging.info(info)
    ...
    def _stage2_bias_correction(self, val_loader, test_loader):
        if isinstance(self._network, nn.DataParallel):
            self._network = self._network.module
        network_params = [{'params': self._network.bias_layers[-1].parameters(), 'lr': self._lrate,
