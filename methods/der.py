...
class DER(BaseLearner):
    ...
    def __init__(self, args):
        ...
        self._network = DERNet(args['backbone'], args['pretrained'])
    ...
    def _train(self, train_loader, test_loader):
        ...
        if self._cur_task==0:
            ...
            optimizer = optim.SGD(filter(lambda p: p.requires_grad, self._network.parameters()), lr=self._lrate, momentum=0.9, weight_decay=self._weight_decay)
    ...
    def _update_representation(self, train_loader, test_loader, optimizer, scheduler):
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            if epoch%5==0:
                ...
                self._cur_task, epoch+1, self._epochs, losses/len(train_loader),losses_clf/len(train_loader),losses_aux/len(train_loader), train_acc, test_acc)
            else:
                info = 'Task {}, Epoch {}/{} => Loss {:.3f}, Loss_clf {:.3f}, Loss_aux {:.3f}, Train_accy {:.2f}'.format(
                self._cur_task, epoch+1, self._epochs, losses/len(train_loader), losses_clf/len(train_loader),losses_aux/len(train_loader),train_acc)
            prog_bar.set_description(info)
        logging.info(info)
